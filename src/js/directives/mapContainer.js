angular.module('marathon').directive('mapContainer', function ($http) {
    return {
        ...
        link: function link($scope, $element) {
            ...
            var req = $http.get('data/geo/mm2014-42km-geo-accurate.json');
            req.then(function (data) {
                $scope.geodata = data.data;
                var trackLength = d3.geo.length($scope.geodata) * earth_radius;
                var startTime = $scope.runnerData.start_time;

                var b = path.bounds($scope.geodata);
                ...
                //var grays = colors.getRGBGradient(4, ['#EEEEEE', '#777777'].map(colors.parseHEXString));
                var ageAreas = {};
