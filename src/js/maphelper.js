...
var getStepHeight = function(track, distance, seconds, runners_array, start_time, total_distance, step_distance){
...
var getPoints = function(runners_groups, track, age_areas, seconds, start_time, total_distance, step) {
	var boundrect = track.node().getBoundingClientRect();

	if (boundrect.width <= 50 || boundrect.height <= 50) {
		return;
	}
	var data = getBasePoints(track, boundrect, total_distance, step);
	...
	runners_groups.forEach(function(el) {
			age_areas[el.key].attr("d", areas[el.key]);
	});
	return data;
